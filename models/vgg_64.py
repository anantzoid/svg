...
class encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        h4_1 = self.c4_1(h4) #---> extra layer
        h5 = self.c5(self.mp(h4_1)) # 4 -> 1
        return h5.view(-1, self.dim), [h1, h2, h3, h4, h4_1]
...
class decoder(nn.Module):
    ...
    def forward(self, input, _=None):
        ...
        up1 = self.up(d1) # 4 -> 8
        #### Newline
        d2_1 = self.upc2_1(torch.cat([up1, skip[4]], 1))

        d2 = self.upc2(torch.cat([d2_1, skip[3]], 1)) # 8 x 8
        up2 = self.up(d2) # 8 -> 16 
...
class decoder_noskip(nn.Module):
    ...
    def forward(self, input, _=None):
        ...
        d1 = self.upc1(vec.view(-1, self.dim, 1, 1)) # 1 -> 4
        up1 = self.up(d1) # 4 -> 8
        d2_1 = self.upc2_1(up1)
        d2 = self.upc2(d2_1) # 8 x 8
        up2 = self.up(d2) # 8 -> 16 
