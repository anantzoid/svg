...
'''
Script taken from https://github.com/anantzoid/svg/blob/29b6d75cb6fe8ea7983bb451d6e1d7c89c108412/parallel_original_lp_mask.py
# since latest script if just copying past frome for gen, could be a laplacian bug as well. didn't have time to check
'''
import torch
...
parser.add_argument('--laplacian', default=0, type=int, help='set_device')
parser.add_argument('--lapweight', default=1.0, type=float, help='set_device')
...
def plot_rec(x, epoch, _type):
    ...
    for i in range(1, opt.n_past+opt.n_future):
        ...
        if i < opt.n_past:
            ...
            x_pred = decoder([h_pred, skip])[0].detach()
...
def train(x):
    ...
    for i in range(1, opt.n_past+opt.n_future):
        ...
        h_pred, all_hidden1 = frame_predictor(torch.cat([h, z_t], 1), all_hidden1)
        x_pred, scales = decoder([h_pred, skip])

        if opt.laplacian ==  1:
            laploss += lap(x_pred, scales, x[i]) 
    ...
    loss = mse + laploss*opt.lapweight + kld*opt.beta
    loss.backward()
